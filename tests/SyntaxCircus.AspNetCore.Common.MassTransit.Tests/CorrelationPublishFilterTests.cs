...
public sealed class CorrelationPublishFilterTests : IDisposable
{
    public void Dispose()
    {
    ...
    private static CorrelationPublishFilter<TestMessage> CreateFilter(string headerName = "X-Correlation-Id")
    {
        var options = Options.Create(new CorrelationIdOptions { HeaderName = headerName });
        return new CorrelationPublishFilter<TestMessage>(options);
    }
